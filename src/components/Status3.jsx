...
const updateStatus = (e, id) => {
    ...
    const updatedPlayerList = playerList.map((player) => {
        if (player._id === id){
            player.game3status = e.target.name
            ...
            <tbody>
                ...
                playerList.map((player, idx) => {
                    return(
                        <tr key={idx}>
                            <td>{player.name}</td>
                            <td><button name="playing" onClick={(e)=>{updateStatus(e, player._id)}} style={player.game3status === 'playing' ? {backgroundColor:"green"} : {backgroundColor:"white"} }>Playing</button> | <button name="notPlaying" onClick={(e)=>{updateStatus(e, player._id)}} style={player.game3status === 'notPlaying' ? {backgroundColor:"red"} : {backgroundColor:"white"} }>Not Playing</button> | <button name="undecided" onClick={(e)=>{updateStatus(e, player._id)}} style={player.game3status === 'undecided' ? {backgroundColor:"yellow"} : {backgroundColor:"white"} }>Undecided</button></td>
                        </tr>
                    )
                    })
                }
